...
static void GenerateHelloBroadcast(Ptr<WifiNetDevice> wifinetdevice, uint32_t pktSize, Time pktInterval, uint32_t numPackets)
{
  if (numPackets>0)
  {
    ...
    NS_LOG_UNCOND ("%INFO: Sending Broadcast packet! I am node "<<wifinetdevice->GetNode ()->GetId ()<<" my MAC is: "<<wifinetdevice->GetAddress());
    // Create packet
    ...
    NS_LOG_UNCOND ("%INFO: Packet sent Generate traffic Broadcast call done");
...
int main (int argc, char *argv[])
{
  ...
  // NS_LOG_DEBUG_, NS_LOG_WARN and LOG_ERROR era LOG_LEVEL_ALL
  LogComponentEnable ("WifiSimpleAdhoc", LOG_LEVEL_ALL);
  ...
  wifiMac.SetType ("ns3::EssoaWifiMac","QosSupported", BooleanValue (false));
  ...
  //BasicEnergySourceHelper basicSourceHelper = BasicEnergySourceHelper () ;
  ...
  //basicSourceHelper.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (5));
  ...
  // Both inherits from DeviceEnergyModelHelper
  ...
  AnimationInterface anim ("Manet_TxRx_24_march_2017.xml");
  ...
  for (uint32_t i = 0; i < nDevices; ++i)
  {
    ...
    //Ptr<WifiRadioEnergyModel> basicRadioModelPtr = basicSourcePtr->GetObject<WifiRadioEnergyModel>();
