...
static void GenerateTraffic (Ptr<WifiNetDevice> wifinetdevice, uint32_t pktSize, uint32_t pktCount, Time pktInterval )
{
  if(pktCount > 0)
  {
    ...
    NS_LOG_UNCOND ("%INFO: generatetrafic");
  }
  else{}
}
...
int main (int argc, char *argv[])
{
 ...
 // double rss = -80;  // -dBm
  ...
  wifiChannel.AddPropagationLoss ("ns3::TwoRayGroundPropagationLossModel","SystemLoss", DoubleValue(1), "HeightAboveZ", DoubleValue(5));
  ...
  positionAlloc->Add (Vector (5.0, 0.0, 5.0));
  ...
  for (uint32_t i = 0;i < nDevices; ++i)
  {
    ...
    // Randomly generate mac addresses
    p->SetAddress(Mac48Address::Allocate ());  
    // Create callbacks for RSS and MAC Rx
  ...
  // Just node 0 will generate trafic everything else will be answers to that first message by the Rx Callback
  ...
  // enable packet capture tracing and xml for netanim
